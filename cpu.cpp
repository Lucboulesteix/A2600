...
void Processor::cpu_tick(Memory mem) { //main loop for CPU
	// at beginning of cycle, 6502 state is defined
	unsigned char opcode = mem.read(PC); //get current opcode from current PC
	printf("Opcode: %x ; PC: %x ; step: %d ; flags N:%d Z:%d, C:%d, I:%d, D:%d, V:%d \n", opcode, PC, step, N,Z,C,I,D,V);
...
Processor::Processor(Memory mem) { //constructor
	Processor::reset(mem);//reset at system startup
}

void Processor::reset(Memory mem) { //reset state to startup
...
void Processor::compute(unsigned char opcode, Memory mem){ // we have a valid opcode and the processor is expected to work (last clock tick)
	//value declaration
	...
	bool PC_cross_PB; //flag if PC has crossed page boundary post jump
	switch (opcode) {
	/*########################### Add with Carry(ADC) ######################################## */
	...
	case 0x90: // BCC relative, 2b 2c++
		...
		if (C == 0) {
			...
			PC = PC + offset; //jumping to target
	...
	/* ######## Test Buts in Memory with accumulator (BIT) #############*/
	...
	case 0xE0: //CPX imm, 2b 2c
		step = 2;
		imm = mem.read(PC + 1);
		C = (X >= imm); //setting carry if A value is gretaer than memory contents
...
/* Store accumulator in memory (STA) ####*/
	...
	case 0x81: //LDA preindirect x, 2b 6c
		step = 6;
	...
	case 0x91: //LDA postindirect t, 2b 6c
		step = 6;
